...
type Product struct {
	...
	Price float64 `db:"normal_price" json:"product_price"`
}
...
func GetProductHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		fmt.Fprintf(w, "ada error nih: %s", err.Error())
		return
	}
	...
	//OPTIONAL : json pakai struct
	hasilJson, err := json.Marshal(productData)
	if err != nil {
		fmt.Fprintf(w, "ada error parsing json: %s", err.Error())
		return
	}

	//OPTIONAL : json pakai map
